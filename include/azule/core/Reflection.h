...
namespace azule
{
	...
	public:
		...
		bool setValue(const impl::AnyRef& x, const std::string& v) override
		{
			...
			if(obj != nullptr)
			{
				obj->*property = azule::StringTo<A>(v);
				return true;
			}
	...
	public:
		...
		bool setValue(const impl::AnyRef& x, const std::string& v) override
		{
			...
			if(obj != nullptr)
			{
				(obj->*setter)(azule::StringTo<Arg>(v));
				return true;
			}
	...
	private:
		...
		std::vector<azule::SharedLibrary> libraries;
